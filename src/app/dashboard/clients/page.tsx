...
export default function ClientsPage() {
  const [clients, setClients] = useState<ClientWithPlanStats[]>([]);
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1200px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <button id="add-client-btn" onClick={openAdd} className="btn btn-primary">
      ...
      {loading ? (
        <div className="table-container">
          <table>
            <thead><tr><th>Name</th><th>Contact</th><th>Plans</th><th>Invested</th><th>Added</th><th></th></tr></thead>
            <tbody>
              {[1, 2, 3].map(i => (
                <tr key={i}>
                  {[1, 2, 3, 4, 5, 6].map(j => (
                    <td key={j}><div className="shimmer" style={{ height: '20px', borderRadius: '4px', width: j === 1 ? '140px' : j === 6 ? '60px' : '80px' }} /></td>
                  ))}
                </tr>
              ))}
            </tbody>
